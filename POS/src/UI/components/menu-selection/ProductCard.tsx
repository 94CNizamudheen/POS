...
export default function ProductCard({ product, onAdd }: ProductCardProps) {
  ...
  return (
    <div
      ref={cardRef}
      onClick={handleAdd}
      className="bg-surface-raised rounded-2xl flex flex-col shadow-sm border border-subtle hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer overflow-hidden"
    >
      <img
        src={imageUrl}
        alt={product.name}
      ...
      <div className="p-3 flex flex-col gap-2">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            className="px-3 py-1 text-xs font-bold rounded-lg bg-surface-sunken text-secondary hover:bg-success hover:text-white transition-all"
            onClick={(e) => {
              e.stopPropagation();
              handleAdd();
            }}
